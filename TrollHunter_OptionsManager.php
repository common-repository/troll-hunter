...
/*
    ...
    This file is part of Troll Hunter By Gavagai for WordPress.
...
class TrollHunter_OptionsManager {
    ...
    /**
     * Call the tonality api with empty body in order to verify that the apiKey is set correctly
     * @param $apiKey
     * @return true if answer is ok
     */
    public function checkApiKey($apiKey){
        $service_url = 'https://api.gavagai.se/v3/tonality?language=en&affiliate=troll-hunter&apiKey=' . $apiKey;
        ...
        curl_close($http);
        error_log("Checking api key: " . $apiKey . " got status: " . $http_status . " with message" . $result);
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'troll-hunter'));
        }
        ...
        if ($optionMetaData != null) {
            foreach ($optionMetaData as $aOptionKey => $aOptionMeta) {
                if (isset($_POST[$aOptionKey])) {
                    if($aOptionKey == 'apiKey') {
                        if(!$this->checkApiKey($_POST[$aOptionKey])) {
                            wp_die(__('PROBLEM WITH KEY.', 'troll-hunter'));
                        }
                    }
        ...
        $settingsGroup = get_class($this) . '-settings-group';
        ?>
        <a href="http://gavagai.se" id="logo-link">
            <img id="heading-logo" src="http://gavagai.se/wp-content/themes/gavagai/img/gavagai-logo.png" alt="Gavagai" title="Gavagai"/>
        </a>

        <div class="wrap">

            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'troll-hunter'); ?></h2>
            <p><?php _e("To use the Troll Hunter you need an apiKey which you can fetch from Gavagai. If you already have a key, paste it below.", 'troll-hunter');?> </p>
            <form method="post" action="">
            <?php settings_fields($settingsGroup); ?>
                <table class="form-table"><tbody>
                    <tr>
                        <th scope="row"">
                            <p><?php _e("Generate an apiKey", 'troll-hunter'); ?></p>
                        </th>
                        <td>
                            <a href="https://developer.gavagai.se/signup?affiliate=troll-hunter" target="_blank"><?php _e("Fetch a key from Gavagai","troll-hunter");?></a>
                        </td>
                        </tr>
                <?php
                if ($optionMetaData != null) {
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'troll-hunter') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'troll-hunter');
            case 'false':
                return __('false', 'troll-hunter');

            case 'Administrator':
                return __('Administrator', 'troll-hunter');
            case 'Editor':
                return __('Editor', 'troll-hunter');
            case 'Author':
                return __('Author', 'troll-hunter');
            case 'Contributor':
                return __('Contributor', 'troll-hunter');
            case 'Subscriber':
                return __('Subscriber', 'troll-hunter');
            case 'Anyone':
                return __('Anyone', 'troll-hunter');
        }
        return $optionValue;
    }
